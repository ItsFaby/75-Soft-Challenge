...
class FirebaseService {
    ...
    async resetAllData() {
        try {
            ...
            const logsSnapshot = await this.db.collection(this.collections.dailyLogs).get();
    ...
    async initializeDefaultUsers() {
        try {
            ...
            Object.entries(AppConfig.USERS).forEach(([userName, userConfig]) => {
                const userRef = this.db.collection(this.collections.users).doc(userName);
                batch.set(userRef, {
                    name: userName,
                    id: userConfig.id,
                    points: 0,
                    personalChallenge: userConfig.personalChallenge,
                    penaltyDescription: userConfig.penaltyDescription,
                    stats: {
                        totalDays: 0,
                        perfectDays: 0,
                        currentStreak: 0,
